...
for tryout = 1:num_pts
    ...
    [db_trained, calculation_time] = Finito_async_lock_free(x, y, alpha, s, tryout, numthread);
    times(tryout) = calculation_time;
    function_values(tryout) = f(db_trained', x, y, s);

end

save(sprintf("async_parallel,n=%d,dim=%d,s=%.1f,finito.mat",n,dim,s), 'times', 'function_values');
...
saveas(ref,sprintf("time-epoch,async_parallel,n=%d,dim=%d,s=%.1f,finito.jpeg",n,dim,s),'jpeg');
close(ref);
...
saveas(fvEpoch,sprintf("fv-epoch,async_parallel,n=%d,dim=%d,s=%.1f,finito.jpeg",n,dim,s),'jpeg');  
close(fvEpoch);
...
saveas(fvTime,sprintf("fv-time,async_parallel,n=%d,dim=%d,s=%.1f,finito.jpeg",n,dim,s),'jpeg');  
close(fvTime);