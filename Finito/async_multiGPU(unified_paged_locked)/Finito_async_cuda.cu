...
int main()
{
  ...
  CUDA_CALL(cudaMallocHost(&d_z_a, sizeof(double) * n * dim));
  CUDA_CALL(cudaMallocHost(&d_mean_z, sizeof(double) * dim));  
  memset(d_z_a, 0, sizeof(double) * n * dim);
  memset(d_mean_z, 0, sizeof(double) * dim);

  // try to reside on host
  int* d_itr_ptr;
  CUDA_CALL(cudaMallocHost(&d_itr_ptr, sizeof(int)));
  ...
  cudaFreeHost(d_itr_ptr);
  cudaFree(d_states0);
  cudaFree(d_states1);
  cudaFreeHost(d_z_a);
  cudaFreeHost(d_mean_z);
  cudaFree(d_x0_a);
  cudaFree(d_y0);
  cudaFree(d_x1_a);
  cudaFree(d_y1);
