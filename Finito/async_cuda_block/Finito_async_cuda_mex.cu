                          ...
                          long long seed)
{
  ...
  __shared__ double sum_buffer[1024/WARP_SIZE];
  ...
  while (*itr_ptr < epoch * n) {
    ...
    for (int c = threadIdx.x; c < dim; c += BLOCKSIZE) 
      dot += mean_z[c] * x_a[dim * ik + c];
    ...
    if (lane == 0)
      sum_buffer[threadIdx.x / WARP_SIZE] = dot;

    __syncthreads();

    unsigned mask = __ballot_sync(0xffffffff, threadIdx.x < BLOCKSIZE / WARP_SIZE);
    if (threadIdx.x < BLOCKSIZE / WARP_SIZE) {
      dot = sum_buffer[threadIdx.x];
      for (int delta = BLOCKSIZE / WARP_SIZE / 2; delta > 0; delta /= 2)
        dot += __shfl_down_sync(mask, dot, delta);
    }
